...
class FHIRTranslator(
    ...
    /**
     * Accepts a FHIR [message], parses it, and generates translated output files for each item in the destination's
     *  element.
        ...
        try {
            ...
            receivers.forEach { recName ->
                ...
                if (receiver != null && receiver.topic == Topic.FULL_ELR.json_val) {
                    val converter = FhirToHl7Converter(bundle, receiver.translation.schemaName)
                    val hl7Message = converter.convert()

                    // TODO Set processing mode to T for CustomerStatus.TESTING or a given VALID processing mode code
                    ...
                    throw IllegalStateException("Unable to find full-ELR receiver $recName in settings.")
                }
            }
