...
resource "azurerm_storage_share" "sftp_v2" {
  for_each = toset(local.users)

  name                 = "${var.resource_prefix}-share-${each.value}"
  storage_account_name = var.storage_account_v2.name
  quota                = 50
}

resource "azurerm_container_group" "sftp_v2" {
  name                = "${var.resource_prefix}-continst-v2"
  resource_group_name = var.resource_group
  location            = var.location
  ip_address_type     = "Private"
  //dns_name_label      = "${var.resource_prefix}-v2"
  network_profile_id = var.network_profile_id
  os_type            = "Linux"
  restart_policy     = "Always"
  ...
  container {
    ...
    volume {
      ...
      mount_path           = "/etc/sftpadmin"
      read_only            = false
      share_name           = "pdh${var.environment}-sftp-admin-v2-share"
      storage_account_name = var.storage_account_v2.name
      storage_account_key  = var.storage_account_v2.primary_access_key
    }
    ...
    volume {
      ...
      mount_path           = "/etc/sftp.d"
      read_only            = false
      share_name           = "pdh${var.environment}-sftp-scripts-v2-share"
      storage_account_name = var.storage_account_v2.name
      storage_account_key  = var.storage_account_v2.primary_access_key
    }
